...
\eref{eq:acoustic-tensor}, which can then be checked for material
bifurcation.
...
\begin{figure}[htbp]
  ...
  \caption{Simple shear test on small deformation isotropic damage
    model: (a) stress strain behavior, with the cross indicating
...
\begin{equation}\label{eq:aniso_tangent}
  \tilde{\bbC}
   = (1-\xi_m) \tilde{\bbC}_m^0
   + \sum_i^n (1-\xi_i) \tilde{\bbC}_i^0
...
It should be noted that the fourth-order tangent $\tilde{\bbC}$
...
tangent $\bbC$ to be computed as a derivative with respect to
...
\begin{equation}\label{eq:aniso_tangent3}
  \bbC_{ijkl} = S_{lj} \delta_{ik}
    + F_{ip} \tilde{\bbC}_{pjlq} F_{kq}
\end{equation}
...
Newton iterative scheme, which may cost more time to arrive at a
