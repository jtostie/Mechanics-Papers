...
instance, \citet{Schreyer.Neilsen:1996} derived an analytical criteria
for the loss of ellipticity for a general class of symmetric
constitutive equations. \citet{Becker:2002} used the Gurson
constitutive model in combination with failure criteria based on
...
has a singularity at the poles in that the azimuthal angle $\theta$
can adopt any value there. This leads to numerical diffculties if
minima happen to lie near or at the poles. The Cartesian
parametrization advocated here does not contain any singularities of
this kind.
...
\begin{equation}\label{eq:iso-psi}
  ...
  :=
  \frac{1}{2} (1 - \xi)
  ~\epsilon^{\text{e}} : \mathbb{C}^{\text{e}} : ~\epsilon^{\text{e}}
\end{equation}
where $~\epsilon^{\text{e}}$ is the infinitesimal elastic strain tensor,
...
computational cost, the Cartesian parametrization is efficient with
...
strain-energy function proposed in \citep{Holzapfel.etal:2010}
\begin{equation}
  \Psi^\fiber_i(~C, ~M)
    = \frac{k_i}{q_i}
    \{ \exp[q_i(I_4 - 1)^2] \}
