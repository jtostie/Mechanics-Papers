...
evolution \eref{eq:iso_xi}, the fourth-order tangent moduli tensor can
...
tensor \eref{eq:acoustic-tensor}, which can then be checked for
...
plotted in \fref{fig:iso_stress_strain}(a). The softening response is
...
\fref{fig:iso_stress_strain}(b) shows the degradation of the
...
$\epsilon_{12}=0.0559$ as marked in \fref{fig:iso_stress_strain}
...
\fref{fig:iso_stress_strain}(b) provides little information on their
...
$\bepsilon_{12}=0.0559$ are shown in \fref{fig:iso_shear_detA}. The
corresponding plane views of the determinant landscapes are shown in
\fref{fig:iso_shear_detAXplane}, where the white stars indicate global
...
\fref{fig:iso_shear_robust} shows the contour of the determinant
...
the derivatives of the damage parameters defined in \eref{eq:iso_xi}
...
\eref{eq:aniso_tangent} is computed as the second derivative of the
...
shown in \fref{fig:aniso_stress_stretch}(a), where the X denotes the
...
the detection of bifurcation. \fref{fig:aniso_stress_stretch}(b) shows
...
\fref{fig:aniso_spherical_detA} to \fref{fig:aniso_cartesian_detA}.
