...
first. Then, the material model is subjected to simple shear to
...
\begin{figure}[htbp]
   ...
   \caption{Plane views of $\det~A$ landscapes at bifurcation for
     simple shear for the small deformation isotropic damage model.
     The white stars indicate global minima.}
...
Evidence for this can be seen in Table~\ref{tab:iso-shear-runtime} as
...
\fref{fig:iso-shear-robust} shows the contour of the determinant
...
cross ($\times$). This figure provides a very direct visualization of
...
\fref{fig:aniso-spherical-detA} to \fref{fig:aniso-cartesian-detA}.
...
As can be seen from Figure~\ref{fig:aniso-cartesian-detA}, the
...
using alternative parametrizations shown in
\fref{fig:aniso-spherical-detA} for the spherical,
\fref{fig:aniso-stereographic-detA} for the stereographic and
\fref{fig:aniso-tangent-detA} for the tangent parametrizations.
...
scheme and eliminating the initial sampling. A total of one thousand
random initial guesses are generated for each parametrization. The
success rate and computation time are recorded and summarized in
...
for the analysis of material instability. In addition, we introduce a
new parametrization that we term Cartesian for the representation of
the normal vector that defines the acoustic tensor in terms of the
tangent moduli tensor. We demonstrate with numerical examples that the
Cartesian parametrization offers the best performance in terms of
computational efficiency and robustenss as compared with other four
