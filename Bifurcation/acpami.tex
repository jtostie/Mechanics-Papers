...
\textcolor{blue}{
This first example illustrates the performance of different parametrizations on a very simple small deformation material model with tangents possessing both major and minor symmetries. For such material model, an alternative single stage approach as in \cite{Oliver.etal:2010} could also be applied, which is based on the solution of a coupled eigenvalue problem in terms of the acoustic tensor. A preliminary analysis shows that the approach in \cite{Oliver.etal:2010} is able to correctly detect bifurcation and is more computationally efficient than the two-stage approach (initial sampling and iterative Newton's solve). More complex material tangents, such as finite deformation models, will be investigated next.
}
...
number of sampling points. \textcolor{blue}{In addition to the total 
computation time, the time for the initial sampling stage and the iterative 
Newton solve are also reported. In general, as the number of sampling
points decreases, so does the total computation time.} Fewer sampling
...
time to arrive at a converged solution. \textcolor{blue}{Also, it is 
interesting to observe that, except for the very dense sampling case 
(interval=0.05), most of the computation time are spent on the second 
stage, i.e., the iterative Newton's solve.} Due to the added complexity 
of the material model, different parametrizations are more sensitive to
...
case, are computationally more expensive. \textcolor{blue}{The 
Cartesian is the most efficient, in terms of the total computation time 
and the time spent on the iterative Newton solve, and at the same time, 
...
\color{blue}
  \begin{center}
    ...
    \caption{Computation time for different parametrizations in the
      ...
      two numbers in the bracket correspond to the time for initial 
      sampling and time for the iterative Newton solve, respectively.}
    \label{tab:aniso-uniaxial-runtime}
  \end{center}
\end{table}
...
\begin{figure}[!htbp]
 ...
 In this finite deformation example, the single-stage approach
 \cite{Oliver.etal:2010} cannot correctly detect the bifurcation time
 nor the bifurcation direction.
...
becomes critical. \textcolor{blue}{It should be noted that, in this finite 
deformation example, the single-stage approach \cite{Oliver.etal:2010} 
cannot correctly detect the bifurcation time nor the bifurcation 
direction.} The Cartesian parametrization thus provides a
