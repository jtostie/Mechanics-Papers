...
\begin{equation}\label{eq:aniso_tangent}
  \tilde{\mathbb{C}} 
   = (1-\xi_m) f_m \tilde{\mathbb{C}}_m^0 
   + \sum_i^n (1-\xi_i) f_i \tilde{\mathbb{C}}_i^0
   -\beta_m \xi_m'  (f_m\bS_m^0 \otimes f_m\bS_m^0 )
   - \sum_i^n \beta_i \xi_i' (f_i \bS_i^0 \otimes f_i \bS_i^0 )
\end{equation}

where $\bS_m^0$ and $\bS_i^0$ are the effective (undamaged) second 
Piola-Kirchhoff stress for the matrix and the $i$th fibers, 
respectively. $\beta_m = 1$ if damage of the matrix evolves within the 
...
thermodynamic force defined in \eref{eq:alpha}. 

The effective tangent modulus tensor is given by
...
\begin{equation}\label{eq:aniso_tangent3}
  \mathbb{C}_{ijkl} = f S_{lj}^0\delta_{ik}
    + F_{ip} \tilde{\mathbb{C}}_{pjlq} F_{kq}
\end{equation}

where $S_{lj}^0$ is the effective second Piola-Kirchhoff stress of the 
corresponding phase (e.g., matrix or fiber component), $f$ is the 
corresponding volume fraction, and $\bdelta$ is the 
Kronecker delta and $\bF$ is the deformation gradient. Note that in 
\eref{eq:aniso_tangent3}, the indices in the subscript refer to the 
individual component of a tensor, not to confuse with the subscripts 
'$m$' and '$i$' used in \eref{eq:aniso_tangent} to represent different 
phases.
