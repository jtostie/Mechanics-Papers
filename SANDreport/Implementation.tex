...
STEP 1. Compute trial elastic left Cauchy-Green tensor $\bb_e^{\tr}$ \eqref{eq:betr}\\
STEP 2. Compute trial Kirchhoff pressure and deviatoric tensor $p^{\tr}, \bs^{\tr}$ \eqref{eq:ptr}, \eqref{eq:str}\\
STEP 3. Check yielding \eqref{eq:Phi}: $\Phi^{\tr}(p^{\tr}, \bs^{\tr}, \varepsilon_{q(n)}, f_n) > 0$ ?\\
$~~~~~~~~~~~~~~$No, set $p = p^{\tr}, \bs = \bs^{\tr}, \bb_e = \bb_e^{\tr}, \varepsilon_{q} = \varepsilon_{q(n)}, f = f_n$  and exit\\
STEP 4. Yes, local Newton loop\\
$~~~~~~~~~~~~~~$4.1 Initialize $\bX^{k}$ \eqref{eq:X} and iteration count $k=0$\\
$~~~~~~~~~~~~~~$4.2 Assemble residual $\bR(\bX^k)$ \eqref{eq:R1} - \eqref{eq:R4}\\\
